...
Map::Map(int mapSize, std::map<int, Node *> nodes, std::map<int, Road> roads, std::vector<Link> links) {
    ...
    for(auto node : nodes) {
            ...
            if (type == NodeType::PARKING_GARAGE || type == NodeType::PARKING_LANE) {
                double cost = type == NodeType::PARKING_GARAGE ? ((ParkingGarage *) node.second)->getCost() : ((ParkingMeter *) node.second)->getCost();
        ...
        for(auto link : links) {
            ...
            double distance = sqrt(pow(n1->getLatitude() - n2->getLatitude(), 2) + pow(n2->getLongitute() - n2->getLongitute(), 2));
...
std::vector<Node *> Map::findShortestPath(int sourceId, int destId, int maxDistance, bool visitGasStation){
    this->parkingSpotSearcher.findShortestPath(sourceId, destId, maxDistance, visitGasStation);
}
